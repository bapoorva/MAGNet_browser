...
server <- function(input, output,session) {
  ...
  output$projects = renderUI({
    excel=readexcel()
    prj=excel$projects
    selectInput("projects","Select an RNAseq project",as.list(sort(as.character(prj))))
  })
  ...
  output$contrasts = renderUI({
    ...
    selectInput("contrast","Select a comparison for RNA-Seq",contrasts,"pick one")
  })
  ...
  plotbiplot = reactive({
    ...
    v = results$eset
    pData<-phenoData(v)
    column=eval(parse(text=paste("pData$",input$habbilage,sep="")))
    validate(
      need(input$pcslide, "Enter number of genes to view in biplot")
    )
    if(input$pcslide==0 & input$ellipse==F){
      fviz_pca_ind(res.pca, repel=T,geom='point',label='var',addEllipses=FALSE, habillage = as.factor(column),pointsize = 3.35,axes=c(x,y))+scale_shape_manual(values = c(rep(19,length(unique(pData$maineffect)))))+theme(axis.title.x = element_text(face="bold", size=14),
      ...
      fviz_pca_ind(res.pca, repel=T,geom='point',label='var',addEllipses=T,ellipse.type="confidence",ellipse.alpha=0.2, habillage = as.factor(column),pointsize = 3.35,axes=c(x,y))+scale_shape_manual(values = c(rep(19,length(unique(pData$maineffect)))))+theme(axis.title.x = element_text(face="bold", size=14),
                                                                                                                                                                                                                                                                             axis.title.y = element_text(face="bold", size=14),
                                                                                                                                                                                                                                                                             legend.text  = element_text(angle=0, vjust=0.5, size=14),
                                                                                                                                                                                                                                                                             legend.title  = element_text(angle=0, vjust=0.5, size=14),
                                                                                                                                                                                                                                                                             plot.title  = element_text(angle=0, vjust=0.5, size=16))
    ...
    else if(input$pcslide!=0 & input$ellipse==F){fviz_pca_biplot(res.pca,repel=T, label='var',habillage = as.factor(column),pointsize = 3.35,axes=c(x,y),select.var = list(contrib = as.numeric(input$pcslide)))+scale_shape_manual(values = c(rep(19,length(unique(pData$maineffect)))))+theme(axis.title.x = element_text(face="bold", size=14),
                                                                                                                                                                                                                                                                                                                   axis.title.y = element_text(face="bold", size=14),
                                                                                                                                                                                                                                                                                                                   legend.text  = element_text(angle=0, vjust=0.5, size=14),
                                                                                                                                                                                                                                                                                                                   legend.title  = element_text(angle=0, vjust=0.5, size=14),
                                                                                                                                                                                                                                                                                                                   plot.title  = element_text(angle=0, vjust=0.5, size=16))
    ...
    else{fviz_pca_biplot(res.pca,repel=T, label='var',addEllipses=T,ellipse.type="confidence",ellipse.alpha=0.1,habillage = as.factor(column),pointsize = 3.35,axes=c(x,y),select.var = list(contrib = as.numeric(input$pcslide)))+scale_shape_manual(values = c(rep(19,length(unique(pData$maineffect)))))+theme(axis.title.x = element_text(face="bold", size=14),
                                                                                                                                                                                                                                                                                                                                     axis.title.y = element_text(face="bold", size=14),
                                                                                                                                                                                                                                                                                                                                     legend.text  = element_text(angle=0, vjust=0.5, size=14),
                                                                                                                                                                                                                                                                                                                                     legend.title  = element_text(angle=0, vjust=0.5, size=14),
                                                                                                                                                                                                                                                                                                                                     plot.title  = element_text(angle=0, vjust=0.5, size=16))
  ...
  res_pca = reactive({
    ...
    m=m %>% dplyr::select(colnames(v.filter))
  ...
  varheatmap <- function(){
    ...
    if(input$hmpsamp==F){
      ...
      pd=pData(results$eset)
      if(ct1=="AADCM" & ct2=="NF"){
        sample=pd$sample_name[pd$etiology %in% c("DCM","NF") & pd$race=="AA"]
      }else if(ct1=="CauDCM" & ct2=="NF"){
        sample=pd$sample_name[pd$etiology %in% c("DCM","NF") & pd$race=="Caucasian"]
      }else if(ct1=="CauNF" & ct2=="AANF"){
        sample=pd$sample_name[pd$etiology=="NF"]
      }else if(ct1=="CauDCM" & ct2=="AADCM"){
        sample=pd$sample_name[pd$etiology =="DCM"]
      }
      sample=as.character(sample)
      top_expr=top_expr[,eval(sample)]}
    validate(
  ...
  ###################################################
  ###################################################
  output$eqtllist <- renderUI({
    results=fileload()
    vals=names(results$eQTL)
    selectInput("dataset", label = "Please select a dataset for the eQTL",choices = vals)
  })
  
  getData <- reactive({
    results=fileload()
    data=eval(parse(text=paste("results$eQTL$",input$dataset,sep="")))
    return(data)
  })
  
  output$eQTLTable = DT::renderDataTable({
